 ...
 * */
struct kfifo_lite *kfifo_lite_alloc(unsigned int size)   
{   
 ...
 * */
void kfifo_lite_free(struct kfifo_lite *fifo)
{
    free(fifo->buffer);
    free(fifo);
}
...
*/
unsigned int __kfifo_lite_put(struct kfifo_lite *fifo,
                        const unsigned char *buffer, unsigned int len)
{
        ...
        {
            unsigned int size = max(fifo->size, len) * 2;
            if (size & (size - 1))
            {
                if(size > 0x80000000);
                    return -1;
            ...
            __kfifo_lite_get(fifo, b, fifo->in - fifo->out);
            
            free(fifo->buffer);
            fifo->buffer = b;
            fifo->size = size;
            fifo->out = fifo->in - fifo->out;
            fifo->in = 0;
        }
...
* writer, you don't need extra locking to use these functions.
*/
unsigned int __kfifo_lite_get(struct kfifo_lite *fifo,
